...
RSpec.describe 'Reservation', type: :request do
  ...
  doctor = Doctor.create(name: 'Dr. Moise', speciality: 'General Practitioner', user:)
  reservation = Reservation.create(datetime: '2022-09-09', user:, doctor:)
  ...
  it 'is not valid without a doctor' do
    reservation = Reservation.new(datetime: '2022-09-09', user:)
    expect(reservation).to_not be_valid
  end
  it 'is not valid without a user' do
    reservation = Reservation.new(datetime: '2022-09-09', doctor:)
    expect(reservation).to_not be_valid
  end
  it 'is not valid without a datetime' do
    reservation = Reservation.new(doctor:, user:)
    expect(reservation).to_not be_valid
  end

  it 'is valid with a doctor\'s name longer than 30 characters' do
    reservation = Reservation.new(doctor:, user:, datetime: '2022-09-09')
    doctor.name = 'a' * 31
